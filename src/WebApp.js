            ...
            <Segment basic inverted vertical={false} style={{height: '100vh'}}>
                ...
                <div style={{textAlign: "center", paddingTop: "10px", display: "flex", justifyContent: "center"}}>
                    <div className="loading">
                        <LoadingIcon step={CONNECTING_TO_DATABASE} steps={steps} currentStep={currentStep} currentStepFailed={currentStepFailed} />
                        <p>Connecting to database</p>
                    </div>
                    <div className="loading">
                        <LoadingIcon step={CHECKING_GDS_PLUGIN} steps={steps} currentStep={currentStep} currentStepFailed={currentStepFailed} />
                        <p>Checking GDS plugin</p>
                    </div>
                    <div className="loading">
                        <LoadingIcon step={CHECKING_APOC_PLUGIN} steps={steps} currentStep={currentStep} currentStepFailed={currentStepFailed} />
...
const LoadingIcon = ({step, steps, currentStep, currentStepFailed}) => {
...
class App extends Component {
  ...
  renderIcon(step) {
    ...
    if(step === this.state.currentStep) {
      return currentStepFailed ?
          <LoadingIcon size="big" name='close' color='red' className="loading-icon" /> :
          <Loader active inline  className="loading-icon" size="large" />;
    } else {
      if(stepIndex > currentIndex) {
        return <LoadingIcon size="big" name='circle notch' color='grey' className="loading-icon" />
      } else {
        return <LoadingIcon size="big" name='checkmark' color='green' className="loading-icon" />
      }
    }
  }
